...
直接跨網域讀取內容（沒有 CORS header），網頁固定改用相對路徑 `/api/news`，只有 `nginx.conf`
有代理這個路徑，也就是只有透過 `docker compose --profile web up webserver` 開啟本機網頁
伺服器瀏覽時才會顯示真新聞標題；直接雙擊開啟 `output/disposal.html` 檔案、或透過 GitHub
Pages 瀏覽，都只會顯示下方的查詢連結卡片。查不到標題時會自動退回顯示下方的查詢連結卡片
（Google 新聞搜尋、Yahoo 奇摩股市、Goodinfo），不會整頁掛掉或顯示錯誤畫面。
